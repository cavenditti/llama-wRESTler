...
class AggregatedExecutionResult(BaseModel):
    ...
    def get_flaky_steps(self) -> list[str]:
        ...
        return [step_id for step_id, stats in self.step_stats.items() if stats.is_flaky]
...
class BatchRecap(BaseModel):
    ...
    common_patterns: list[str] = Field(
        default_factory=list, description="Common patterns observed across endpoints"
    )
...
class FullAnalysisRecap(BaseModel):
    ...
    priority_fixes: list[str] = Field(
        default_factory=list, description="Prioritized list of fixes to attempt"
    )
    flaky_step_notes: str | None = Field(
        None, description="Notes about flaky tests if any were detected"
    )
...
async def run_multiple_executions(
    ...
    for run_idx in range(num_runs):
        ...
        for step_result in result.results:
            ...
            if step_result.success:
                ...
                stats.error_frequencies[error_key] = (
                    stats.error_frequencies.get(error_key, 0) + 1
                )

                # Track status code frequencies
                if step_result.status_code is not None:
                    stats.status_code_frequencies[step_result.status_code] = (
                        stats.status_code_frequencies.get(step_result.status_code, 0)
                        + 1
                    )
    ...
    consensus_result = _build_consensus_result(
        test_plan, step_stats, individual_results
    )

    # Copy broken deps from any run (should be consistent)
    if individual_results:
        consensus_result.steps_with_broken_deps = individual_results[
            0
        ].steps_with_broken_deps
...
def _build_consensus_result(
    ...
    for step in test_plan.steps:
        ...
        if (
            ...
            and stats.success_count > stats.skip_count
        ):
            # Majority success - use a successful result
            consensus = next((r for r in step_results if r.success), step_results[0])
            passed += 1
        elif (
            stats.skip_count > stats.success_count
            and stats.skip_count > stats.failure_count
        ):
            # Majority skipped
            consensus = next(
                (
                    ...
                    if r.error == "Skipped due to failed dependencies"
                ),
                step_results[0],
            )
            skipped += 1
            ...
            consensus = next(
                (r for r in step_results if not r.success), step_results[0]
            )
            failed += 1
...
async def generate_failure_recaps(
    ...
    for i in range(0, len(all_failing), batch_size):
        ...
        logger.info(
            "Processing batch %d/%d (%d endpoints)",
            batch_num,
            total_batches,
            len(batch_step_ids),
        )
...
"""
        ...
        if output_dir:
            ...
            request_file = (
                recaps_dir
                / f"recap_batch{batch_num}_request{iter_suffix}_{timestamp}.md"
            )
        ...
        try:
            ...
            if output_dir:
                response_file = (
                    recaps_dir
                    / f"recap_batch{batch_num}_response{iter_suffix}_{timestamp}.json"
                )
                with open(response_file, "w") as f:
                    json.dump(result.output.model_dump(), f, indent=2)
                logger.info(
                    "Saved recap batch %d response to %s", batch_num, response_file
                )
...
def _build_batch_context(
    ...
    for step_id in step_ids:
        ...
        parts.append(
            f"- Success Rate: {stats.success_rate:.0f}% ({stats.success_count}/{stats.total_runs})"
        )
        ...
        if stats.error_frequencies:
            ...
            parts.append(
                f"- Most Common Error: {most_common_error[0]} (seen {most_common_error[1]}x)"
            )
        ...
        if stats.status_code_frequencies:
            status_str = ", ".join(
                f"{code}({count}x)"
                for code, count in stats.status_code_frequencies.items()
            )
        ...
        if stats.sample_failure_response:
            try:
                ...
                parts.append(
                    f"- Sample Error Response: {stats.sample_failure_response}"
                )
